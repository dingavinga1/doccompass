...
## Goal
Expose stable REST endpoints for ingestion control and documentation/section retrieval, including tree navigation and semantic search.
...
- Documentation endpoints:
  ...
  - `GET /documentation/:id/:section_path`
...
- Search endpoint integration with vector queries
- API-level authentication hook points for MCP token checks
...
6. Implement semantic search endpoint returning ranked results and scores.
7. Add delete behavior with safe cascade and idempotent response patterns.
...
3. Auth guard tests for missing/invalid token handling.
4. Pagination tests (boundaries, empty pages, deterministic ordering).
5. Search tests with mocked embedding/query service.
6. Delete tests confirming cascading data removal behavior.
...
4. Execute semantic search queries and spot-check relevance.
